...
오늘은 **ELK stack(Elasticsearch + Logstash + Kibana)**에 대해서 간략히 설명하고, 그 중 가장 먼저 쓰이는 **logstash**에 대해서 이야기해보자.

## ELK Stack이 무엇인가?

**ELK stack**은 위에서 쓴 것처럼 **Elasticsearch**, **Logastash**, **Kibana** 세 개의 오픈 소스 프로젝트를 의미해. 데이터를 수집 및 처리/검색/시각화하기 위해서는 위의 세 가지가 모두 필요하기 때문에 보통 ELK stack이라고불러.
...
위와 같이 logstash를 통해 수집/처리된 데이터는 elasticsearch에 저장/처리되어 검색 및 분석이 가능하게 돼. Elasticsearch에 저장된 데이터는 kibana를 통해 시각화를 할 수 있어.

오늘은 세 가지 중에서 데이터 수집 엔진인 logstash에 대해서 알아볼꺼야.

## Logstash의 강점

<a href="https://i.imgur.com/9yWqvBc"><img src="https://i.imgur.com/9yWqvBc.png" width="450px" title="source: imgur.com" /></a>

위의 그림은 logstash의 강점을 보여줄 수 있는 그림이야. Logstash는 다양한 소스의 데이터를 수집할 수 있으며 또한 다양한 목적지로 출력할 수 있어.

이렇게 데이터의 입력/출력에 유연성을 갖는 이유는 다양한 플러그인을 제공하고 있기 때문이야. 플러그인에 대해서는 다음 section에서 살펴보자.

## Logstash의 구성

<a href="https://i.imgur.com/rHoB6Th"><img src="https://i.imgur.com/rHoB6Th.png" width="450px" title="source: imgur.com" /></a>

Logstash의 파이프라인은 input, filter, output로 구성되어 있어. 

 ● input  : 데이터 입력, 여러 시스템에 다양한 형태의 데이터를 입력할 수 있음.<br/>
 ● filter : 데이터 처리, 구문 분석 및 필드 식별 등의 전처리를 하여, 공통 형식으로 통합 가능.<br/>
 ● output : 데이터 출력, 원하는 곳으로 다양한 출력을 할 수 있음.
 

### logstash 구성 파일(.conf)의 형태
...
Logstash에서는 각각 블락에 수많은 플러그인을 제공하고 있어. input에는 file/csv/beats.., filter는 grok/mutate/... 등의 플러그인이 존재해. 이를 통해서 다양한 형태의 데이터를 다루는 유연성/확장성을 가진 엔진으로 사용되고 있는 거지.



## Logstash 실습

우리가 가진 데이터는 다음과 같아. 데이터 파일은 확장자가 없는 파일로 "C:/users" 안에 있다고 하자. 

|ID;DATE;NOTE|
|---------|
|1;20200101;오늘 먹은 메뉴는\r\n 라면|
|2;20201003;수업은 9시, 시작!|
|...;...;...|

위의 데이터를 처리하기 위해서, "logstash-example.conf" 구성파일을 만들어보자.
